...
cocktailCore.style.abstract.AbstractStyle.prototype._transformOrigin = null;
...
cocktailCore.style.abstract.AbstractStyle.prototype._isDirty = null;
...
cocktailCore.style.abstract.AbstractStyle.prototype.isParentDirty = function() {
	var ret = true;
...
cocktailCore.style.computer.boxComputers.EmbeddedBlockBoxStylesComputer.prototype.getComputedAutoHeight = function(style,containingDOMElementData) {
	...
	if(style.getWidth() == cocktail.style.DimensionStyleValue.autoValue) {
		...
		if(embeddedDOMElement.getIntrinsicRatio() != null) {
			ret = Math.round(style.getComputedStyle().width / embeddedDOMElement.getIntrinsicRatio());
		}
		else {
			ret = 150;
		}
...
cocktailCore.style.computer.boxComputers.EmbeddedBlockBoxStylesComputer.prototype.getComputedAutoMargin = function(marginStyleValue,opositeMarginStyleValue,containingDOMElementDimension,computedDimension,isDimensionAuto,computedPaddingsDimension,fontSize,xHeight,isHorizontalMargin) {
	...
	if(isHorizontalMargin == false) {
		...
		computedMargin = cocktailCore.style.computer.BoxStylesComputer.prototype.getComputedAutoMargin.call(this,marginStyleValue,opositeMarginStyleValue,containingDOMElementDimension,computedDimension,false,computedPaddingsDimension,fontSize,xHeight,isHorizontalMargin);
	}
	return computedMargin;
}
...
cocktailCore.style.computer.boxComputers.PositionedBoxStylesComputer.prototype.measureAutoWidth = function(style,containingDOMElementData) {
	...
	if(style.getMarginLeft() == cocktail.style.MarginStyleValue.autoValue) {
		...
		computedStyle.marginLeft = this.getComputedMarginLeft(style,containingDOMElementData);
	}
	if(style.getMarginRight() == cocktail.style.MarginStyleValue.autoValue) {
		computedStyle.marginRight = 0;
	}
	else {
		computedStyle.marginRight = this.getComputedMarginRight(style,containingDOMElementData);
	}
	if(style.getLeft() != cocktail.style.PositionOffsetStyleValue.autoValue && style.getRight() != cocktail.style.PositionOffsetStyleValue.autoValue) {
